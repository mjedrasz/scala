...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.isEmpty) means.groupBy(x => x).mapValues { x => List() }
    else points.groupBy { x => findClosest(x, means) }
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.map(m => findAverage(m, classified.getOrElse(m, List())))
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    !oldMeans.zipWithIndex.exists { case (p, i) => oldMeans(i).squareDistance(newMeans(i)) > eta }
  }
