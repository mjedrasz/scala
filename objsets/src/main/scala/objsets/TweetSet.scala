...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  def descendingByRetweet: TweetList = descendingByRetweet(Nil)

  private def descendingByRetweet(list: TweetList): TweetList = {
    if (isEmpty) {
      list
    } else {
      val elem = mostRetweeted
      remove(elem).descendingByRetweet(new Cons(elem, list))
    }
  }
  ...
  def mostRetweeted(elem: Tweet): Tweet
}
...
class Empty extends TweetSet {
  ...
  def mostRetweeted(elem: Tweet): Tweet = elem
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    left.mostRetweeted(right.mostRetweeted(elem))
  }

  def mostRetweeted(el: Tweet): Tweet = {
    if (isEmpty) {
      elem
    } else if (elem.retweets >= el.retweets){
      left.mostRetweeted(right.mostRetweeted(el))
    } else {
      left.mostRetweeted(right.mostRetweeted(elem))
    }
  }
