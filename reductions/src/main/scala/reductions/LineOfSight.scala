...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    var maxA = 0.0f
    for (i <- 1 to input.size - 1) {
      maxA = max(maxA, input(i)/i)
      output(i) = maxA
    }
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    tree: Tree): Unit = {
    tree match {
      case Leaf(from, until, angle) => downsweepSequential(input, output, angle, from, until)
      case Node(l, r) => {
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float],
    ...
    downsweep(input, output, Float.NaN, tree)
  }
}
